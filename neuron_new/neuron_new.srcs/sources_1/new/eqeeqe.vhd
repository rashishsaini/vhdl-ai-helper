...
entity single_neuron is
    ...
    generic (
        ...
        USE_LINEAR_ACT  : boolean := false; -- Linear activation when true (for output layer)
...
architecture rtl of single_neuron is
    ...
    -- Scales weights by sqrt(2/NUM_INPUTS) to prevent dying ReLU
    -- Different per neuron (via NEURON_ID) to ensure neurons in same layer evolve differently
    function init_weight(neuron_id : integer; weight_idx : integer; num_inputs : integer) return signed is
    ...
    begin
        ...
        val := 2048 - seed;  -- Base range: -2048 to +2048 (approx -0.25 to +0.25)

        -- He initialization: scale by sqrt(2/n_inputs)
        -- For n=4: scale by ~0.71 (multiply by 1448/2048)
        -- For n=8: scale by ~0.50 (multiply by 1024/2048)
        -- Approximation: scale by (2896 / (num_inputs + 4))
        if num_inputs = 4 then
            scaled_val := (val * 1448) / 2048;  -- 0.707 * val
        elsif num_inputs = 8 then
            scaled_val := (val * 1024) / 2048;  -- 0.500 * val
        else
            -- General approximation for other sizes
            scaled_val := (val * 2896) / (num_inputs * 724);
        end if;

        return to_signed(scaled_val, DATA_WIDTH);
    end function;
    ...
    begin
        for i in 0 to num_inputs-1 loop
            result(i) := init_weight(neuron_id, i, num_inputs);
        end loop;
        return result;
    end function;

    signal weights : weight_array_t := init_weights(NEURON_ID, NUM_INPUTS);
    -- Initialize bias to small positive value (+0.1 = 820 in Q2.13) to prevent dying ReLU
                ...
                case bwd_state is
                    ...
                    when BWD_CALC_GRADIENTS =>
                        ...
                        if bwd_clear = '1' then
                            ...
                            -- Bias gradient = Î´ (shifted from Q2.13 to Q4.26 format)
                            -- Shift left by 13 bits to match weight gradient format
