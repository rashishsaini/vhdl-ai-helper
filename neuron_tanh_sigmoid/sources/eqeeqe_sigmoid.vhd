...
entity single_neuron_sigmoid is
...
end entity single_neuron_sigmoid;



architecture rtl of single_neuron_sigmoid is
    ...
    -- σ(x) = 1/(1+e^(-x)) ≈ { 0 for x < -4
    --                        { 0.25*x + 0.5 for -4 ≤ x < 4  (linear around 0)
    --                        { 1 for x ≥ 4
    function sigmoid_approx(x : signed(DATA_WIDTH-1 downto 0)) return signed is
        constant NEG_FOUR : signed(DATA_WIDTH-1 downto 0) := to_signed(-4 * 8192, DATA_WIDTH);  -- -4.0 in Q2.13
        ...
        constant POS_FOUR : signed(DATA_WIDTH-1 downto 0) := to_signed(4 * 8192, DATA_WIDTH);   --  4.0
        constant SIGMOID_MAX : signed(DATA_WIDTH-1 downto 0) := to_signed(8192, DATA_WIDTH);    --  1.0
        constant SIGMOID_MIN : signed(DATA_WIDTH-1 downto 0) := to_signed(0, DATA_WIDTH);       --  0.0
        constant SIGMOID_MID : signed(DATA_WIDTH-1 downto 0) := to_signed(4096, DATA_WIDTH);    --  0.5
        variable result : signed(2*DATA_WIDTH-1 downto 0);
    begin
        if x < NEG_TWO then
            return SIGMOID_MIN;  -- Saturate at 0.0
        elsif x < POS_TWO then
            -- Linear approximation: result = 0.25*x + 0.5
            result := x * to_signed(2048, DATA_WIDTH);  -- 0.25 in Q2.13
            return resize(result(DATA_WIDTH+12 downto 13) + SIGMOID_MID, DATA_WIDTH);  -- Add 0.5
        else
            return SIGMOID_MAX;  -- Saturate at 1.0
        end if;
    end function;

    -- Sigmoid derivative: σ'(x) = σ(x) * (1 - σ(x))
    -- Approximation: For the linear region (-2 to 2), derivative ≈ 0.25
    function sigmoid_derivative(z : signed(DATA_WIDTH-1 downto 0)) return signed is
    ...
    begin
        if z >= NEG_TWO and z <= POS_TWO then
            return to_signed(2048, DATA_WIDTH);  -- 0.25 in Q2.13
        else
            return to_signed(205, DATA_WIDTH);  -- 0.025 in Q2.13 (small gradient at saturation)
        end if;
    end function;
                ...
                case bwd_state is
                    ...
                    when BWD_CALC_DELTA =>
                        ...
                        if bwd_clear = '1' then
                            ...
                            deriv := sigmoid_derivative(z_reg);
